...
Generated output can be used in .github/workflows/sync_transifex.yml
"""
...
# All supported locales for Bisq Mobile (sorted alphabetically)
# Current locales: en (default), af_ZA, cs, de, es, fr, hi, id, it, pcm, pt_BR, ru, tr, vi
ALL_LOCALES = [
    "af_ZA", "cs", "de", "es", "fr", "hi", "id", "it", "pcm", "pt_BR", "ru", "tr", "vi"
]

# Priority tiers for future optimization (when adding more locales)
PRIORITY_TIERS = {
    "critical": ["de", "es", "ru", "fr"],  # High-traffic markets
    "important": ["it", "pt_BR", "hi", "id", "tr", "vi"],  # Secondary markets
...
def print_github_actions_json(batches: List[Dict], resources_map: Optional[Dict[str, str]] = None) -> None:
    ...
    Args:
        ...
        resources_map: Optional mapping of locale -> resources (e.g., {"de": "bisq-mobile.chat_properties"})
    """
    matrix_items = []
...
def print_statistics(batches: List[Dict], max_parallel: int, batch_size: int, total_locales: int):
    ...
    # Estimates for Bisq Mobile (15 resources per locale)
    ...
    # API call calculations (Bisq Mobile has 15 resource bundles)
    resources_per_locale = 15
    ...
    print("-" * 70)
    print(f"Resources per locale:   {resources_per_locale}")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Standard batching (4 locales per batch, 2 parallel)
  python scripts/generate_transifex_batches.py --batch-size 4 --max-parallel 2

  # Priority-based batching (critical locales first)
  python scripts/generate_transifex_batches.py --batch-size 4 --max-parallel 2 --priority

  # Conservative batching (smaller batches, less parallelism)
  python scripts/generate_transifex_batches.py --batch-size 3 --max-parallel 2

  # Generate JSON for specific changed locales
  python scripts/generate_transifex_batches.py --locales "de,es,ru" --json
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of locales per batch (default: 4)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Maximum number of parallel batches (default: 2)"
    )
    ...
    parser.add_argument(
        "--locales",
        type=str,
        help="Comma-separated list of locales to batch (e.g., 'de,es,ru'). Uses internal format (pt_BR). If omitted, uses all locales."
    )
    ...
    parser.add_argument(
        "--resources-json",
        type=str,
        help="JSON string mapping locales to resources (e.g., '{\"de\":\"bisq-mobile.chat_properties\"}'). Used with --json to include resources in output."
    )
    ...
    if args.locales:
        ...
        # Don't validate against ALL_LOCALES since dynamic locales may be passed
