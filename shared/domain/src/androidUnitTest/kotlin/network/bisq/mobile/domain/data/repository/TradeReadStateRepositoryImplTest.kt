...
class TradeReadStateRepositoryImplTest {
    ...
    @Test
    fun `data flow should return trade read state map from datastore`() =
        runTest {
            // Given
            val expectedMap =
                TradeReadStateMap(
    ...
    @Test
    fun `data flow should emit empty map on IOException and log error`() =
        runTest {
            // Given
    ...
    @Test
    fun `setCount should add new trade read state`() =
        runTest {
            // Given
    ...
    @Test
    fun `setCount should update existing trade read state`() =
        runTest {
            // Given
            val updateSlot = slot<suspend (TradeReadStateMap) -> TradeReadStateMap>()
            coEvery { mockDataStore.updateData(capture(updateSlot)) } returns TradeReadStateMap()
    ...
    @Test
    fun `setCount should reject blank tradeId`() =
        runTest {
            // When & Then
    ...
    @Test
    fun `setCount should reject negative count`() =
        runTest {
            // When & Then
            assertFailsWith<IllegalArgumentException> {
    ...
    @Test
    fun `setCount should accept zero count`() =
        runTest {
            // Given
            val updateSlot = slot<suspend (TradeReadStateMap) -> TradeReadStateMap>()
            coEvery { mockDataStore.updateData(capture(updateSlot)) } returns TradeReadStateMap()
    ...
    @Test
    fun `clearId should remove existing trade from map`() =
        runTest {
            // Given
            val updateSlot = slot<suspend (TradeReadStateMap) -> TradeReadStateMap>()
            coEvery { mockDataStore.updateData(capture(updateSlot)) } returns TradeReadStateMap()
    ...
    @Test
    fun `clearId should handle non-existing trade gracefully`() =
        runTest {
            // Given
            val updateSlot = slot<suspend (TradeReadStateMap) -> TradeReadStateMap>()
            coEvery { mockDataStore.updateData(capture(updateSlot)) } returns TradeReadStateMap()
    ...
    @Test
    fun `clearId should reject blank tradeId`() =
        runTest {
            // When & Then
            assertFailsWith<IllegalArgumentException> {
                repository.clearId("")
    ...
    @Test
    fun `multiple operations should work correctly together`() =
        runTest {
            // Given
